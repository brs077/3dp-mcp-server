#!/usr/bin/env python3
"""Left curve (90-degree) track segment for Pixar Cars Mini Racers - 3 lanes wide."""
from build123d import *
import os

OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "track-curve-left")
...
# Curve parameters - curve center at origin, arc from 0 to 90 deg
INNER_RADIUS = 80.0
OUTER_RADIUS = INNER_RADIUS + TOTAL_WIDTH
CENTER_RADIUS = INNER_RADIUS + TOTAL_WIDTH / 2

with BuildPart() as track:
    # Base plate - annular sector via revolve of cross-section
    ...
    # Tongue on entry face (angle=0, extending in -Y from the arc edge)
    ...
    # Groove on exit face (angle=90, extending in -X from the arc edge)
...
# Export
stl_path = os.path.join(OUTPUT_DIR, "track-curve-left.stl")
step_path = os.path.join(OUTPUT_DIR, "track-curve-left.step")
export_stl(result, stl_path)
export_step(result, step_path)

bb = result.bounding_box()
print(f"Left curve track created!")
